...
public class ItemHeadMask extends Item implements IBauble {

	public static final Item RING = null;

	public ItemHeadMask(String name) {
	...
	public void onWornTick(ItemStack itemstack, EntityLivingBase player) {
		if (player.ticksExisted % 19 == 0) {
			...
			AxisAlignedBB box = new AxisAlignedBB(center.x, center.y, center.z, center.x, center.y, center.z).grow(7);
			for (EntityLivingBase entity : player.world.getEntitiesWithinAABB(EntityLivingBase.class, box)) {
				if (!(entity instanceof EntityParasiteBase)) {
					entity.addPotionEffect(new PotionEffect(SRPPotions.EPEL_E, 20*7, 1, false, false));
				}
			}
		}
	}
