...
public class ItemGildedHiveBane extends ItemSword {
		...
		if (player.isSneaking()) {
			if (!world.isRemote) {
				...
				if (stackOther.getItem() instanceof ItemModule) {
					...
					// Temporary until i figure out what to do for the non-parasite modules
	...
	public boolean shouldApple(Kind kind, EntityLivingBase target) {
		if (kind.equals(Kind.INBORN) && isInborn(target)) {
			...
			return true;
		} else if (kind.equals(Kind.DESMOID) && (isInborn(target) || target instanceof EntityPInfected || target instanceof EntityPAssimara || target instanceof EntityPHijacked)) {
			return true;
		} else if (kind.equals(Kind.ESCHAR) && (target instanceof EntityPFeral || target instanceof EntityPCrude || target instanceof EntityPPrimitive)) {
			return true;
		} else if (kind.equals(Kind.RESISTANCE) && (target instanceof EntityPAdapted || target instanceof EntityPBeckon || target instanceof EntityPDispatcher || target instanceof EntityPRooter || target instanceof EntityPStationary)) {
			return true;
		} else if (kind.equals(Kind.IDEAL) && (target instanceof EntityPPure || target instanceof EntityPPreeminent || target instanceof EntityPDerived || target instanceof EntityPAncient)) {
			return true;
	...
	public boolean isInborn(EntityLivingBase target) {
